...
// mutation to save code execution
export const saveExecution = mutation({
  args: {
    language: v.string(),
    code: v.string(),
    // only one of the following would be available
  ...
  handler: async (ctx, args) => {
    ...
    if (!user?.isPro && args.language !== "javascript") {
      throw new ConvexError(
        "Pro Suscription required to run code in this language"
      );
    }
